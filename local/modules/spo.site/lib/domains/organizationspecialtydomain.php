...
class OrganizationSpecialtyDomain extends SPODomain
{
    ...
    public function getOrganizationSpecialtiesApplicationCount()
    {
        ...
        $ArrayResult = OrganizationTable::getList(array(
            ...
            'order'   => array('ORGANIZATION_SPECIALTY.ORGANIZATION_SPECIALTY_ID'=>'ASC'),
            'select' => array(
                'id'=>'ORGANIZATION_SPECIALTY.ORGANIZATION_SPECIALTY_ID',
                //'stat'=>'ORGANIZATION_SPECIALTY.APPLICATION.APPLICATION_STATUS',
                new Entity\ExpressionField('stat', 'COUNT(%s)', array('ORGANIZATION_SPECIALTY.APPLICATION.APPLICATION_STATUS'))
            )
        ))->fetchAll();
        foreach($ArrayResult as $qResultItem) {
