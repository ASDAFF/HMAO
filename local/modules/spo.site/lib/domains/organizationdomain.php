...
class OrganizationDomain extends SPODomain
{
    ...
    public static function loadByEmployeeUserId($userId)
    {
        ...
        /*$organization = OrganizationRepository::create()
            ...
            ->withRegionArea()
            ->one();*/
        if (count($application_data) === null){
            throw new Main\SystemException('Not Found Exception');
        }

        return $application_data[0]['organization_id'];
    }
...
//    }
	...
	{
        $ArrSelect['ORGANIZATION_ID']=$organizationId;
        if($onlyActiveSpecialties!=false){
            $ArrSelect['Status']=$onlyActiveSpecialties;
        }

        $organizationId=OrganizationSpecialtyTable::getList(array(
            'select' => array('ID'=>'ORGANIZATION_SPECIALTY_ID'),
            'filter'       =>  $ArrSelect,
        ))->fetchAll();
		...
		if (count($organizationId) === 0){
			die('Not Found Exception');
        }
    ...
    public static function checkIfUserIsEmployeeOfAnyOrganization($userId){
        $ArrayResult = OrganizationEmployeeTable::getList(array(
            'filter' => array(
                'LOGIC' => 'OR',
                'USER_ID'=>$userId,
                'USER_MODERATOR'=>$userId,
	...
	public function getOrganizationName($requestedOrganizationId)
	{
        ...
        return $ArrayResult[0]['OrganizationName'];
    ...
    public function getStatus($requestedOrganizationId)
    {
        $ArrayResult = OrganizationTable::getList(array(
            'filter' => array(
                'ORGANIZATION_ID'=>$requestedOrganizationId,
        ...
        return $ArrayResult[0]['OrganizationStatus'];
