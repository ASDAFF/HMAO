...
class AdmissionPlanDomain extends SPODomain {
    ...
    public static function changeStatus($admissionPlanId, $newAdmissionPlanStatus, $reason = '')
    {
        ...
        $admissionPlanDomain->persistEntity($planEvent);*/
        AdmissionPlanTable::update($admissionPlanId, array(
            'ADMISSION_PLAN_STATUS' => $newAdmissionPlanStatus
        ));
        global $USER;
        $date2 = date( "d.m.y H:i:s" );
        $date2 = new \Bitrix\Main\Type\DateTime($date2);
        $max = AdmissionPlanEventTable::getList(array(
            'select' => array(
                new \Bitrix\Main\Entity\ExpressionField('MAX_AGE', 'MAX(%s)', array('ADMISSION_PLAN_EVENT_ID')),
                'MAX_AGE'
            )
        ))->fetchAll();
        $max = $max[0]['MAX_AGE']+1;
        $result = AdmissionPlanEventTable::add(array(
            'ADMISSION_PLAN_EVENT_ID' => $max,
            'ADMISSION_PLAN_ID' => $admissionPlanId,
            'ADMISSION_PLAN_EVENT_DATE' =>  $date2,
            'ADMISSION_PLAN_EVENT_COMMENT' => $reason,
            'ADMISSION_PLAN_EVENT_STATUS' => $newAdmissionPlanStatus,
            'USER_ID' => $USER->GetID(),
        ));
        return $result;
    }
