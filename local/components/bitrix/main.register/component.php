...
$params = array(
	'filter' => array(
		'=ORGANIZATION_EMPLOYEE.USER_MODERATOR' => '',
	),
	'select' => array(
		'id' => 'ORGANIZATION_EMPLOYEE.ORGANIZATION_EMPLOYEE_ID',
		'Name' => 'ORGANIZATION_NAME',
	)
);
$resultDb = OrganizationTable::getList($params)->fetchAll();
$arResult['organizacia']=$resultDb;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	...
	else // if there;s no any errors - create user
	{
        ...
        //-----------------------------
		$arResult['VALUES']["maderator"]=$_REQUEST['REGISTER']['maderator'];
		if($arResult['VALUES']["maderator"]!='on') {
			$arResult['VALUES']["Organization"] = $_REQUEST['REGISTER']['Organization'];
			//--------------------------------------
			/*$params = array(
                'filter' => array(
                    '=ORGANIZATION_EMPLOYEE.ORGANIZATION_EMPLOYEE_ID' => $arResult['VALUES']["Organization"],
                ),
                'select' => array(
                    'id' => 'ORGANIZATION_EMPLOYEE.ORGANIZATION_EMPLOYEE_ID',
                )
            ...
            $resultDb = OrganizationTable::getList($params)->fetchAll();
			if(count($resultDb)!=0){
				$arResult['ERRORS']['moderator'] = 1;
			}*/
			//--------------------------------------
		}
		else{
			$arResult['VALUES']["Organization"] = null;
		}
		//-----------------------------
		...
		if (intval($ID) > 0)
		{
			...
			UiMessage::addMessage('Благодарим Вас за регистрацию на портале профессионального образования. Проверьте свою почту! На нее должно было прийти письмо, содержащее инстурцию по активации профиля!', UiMessage::TYPE_WARNING);
			/*if($bConfirmReq)
				$event->SendImmediate("NEW_USER_CONFIRM", SITE_ID, $arEventFields);*/
		}
		else
